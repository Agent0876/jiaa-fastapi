...
# 로드맵 생성 모델 ID
# 
# 옵션 1: Claude 3.5 Sonnet (최신, 고성능)
#   - Inference Profile 필요 (on-demand 미지원)
#   - 환경 변수: ROADMAP_MODEL_ID="arn:aws:bedrock:us-east-1::inference-profile/your-profile-id"
#   - AWS 콘솔에서 Inference Profile 생성 필요
#
# 옵션 2: Claude 3 Sonnet (구버전, on-demand 지원 가능)
#   - 모델 ID: "anthropic.claude-3-sonnet-20240229-v1:0"
#   - Inference Profile 불필요
#
# 옵션 3: Claude 3 Haiku (기본값, on-demand 지원)
#   - 모델 ID: "anthropic.claude-3-haiku-20240307-v1:0"
#   - 가장 빠르고 저렴
#
# 환경 변수 ROADMAP_MODEL_ID로 설정 가능
ROADMAP_MODEL_ID = os.getenv(
    "ROADMAP_MODEL_ID", 
    "anthropic.claude-3-sonnet-20240229-v1:0"  # 기본값: Claude 3 Sonnet (on-demand 지원)
)
...
async def update_roadmap_item(roadmap_item_id: str, request: UpdateRoadmapItemRequest):
    ...
    roadmap_item_id 형식: "roadmap_id:item_index" (예: "507f1f77bcf86cd799439011:0")
    """
    try:
        from bson import ObjectId
        
        # roadmap_id와 item_index 분리
        if ":" not in roadmap_item_id:
            raise HTTPException(status_code=400, detail="잘못된 roadmap_item_id 형식입니다. 'roadmap_id:item_index' 형식이어야 합니다.")
        
        roadmap_id_str, item_index_str = roadmap_item_id.split(":", 1)
        try:
            from bson.errors import InvalidId
            try:
                roadmap_id = ObjectId(roadmap_id_str)
            except (InvalidId, ValueError, TypeError):
                # ObjectId가 아닌 경우 문자열로 직접 사용
        ...
        return {
            "id": item_index,
            "day": updated_item.get("day"),
            "is_completed": bool(updated_item.get("is_completed", 0)),
            "completed_at": updated_item.get("completed_at").isoformat() if updated_item.get("completed_at") else None
        }
    except HTTPException:
        raise
    except Exception as e:
